...
import { getlivingRoomFurniture, addlivingRoomFurnitures, editlivingRoomFurnitures,deletelivingRoomFurnitures } from '../../Api/api'
...
const AdminLivingRoomFurniture = () => {
  ...
  //true(shows addlivingRoomFurnitures pop-up)  false(hide)
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await addlivingRoomFurnitures(livingRoomFurnitures)
      if (response.status === 200) {
        console.log("livingRoomFurnitures Added")
        setShowAdd(false)
        fetchData()
      }
  ...
  const editHelper = (livingRoomFurnitures) => {
    setCurrentlivingRoomFurnitures(livingRoomFurnitures)
    setShowEdit(true)
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editlivingRoomFurnitures(livingRoomFurnitures, currentlivingRoomFurnitures._id)
      if (response.status === 200) {
        setShowEdit(!showEdit)
        fetchData()
        toast.info("livingRoomFurnitures Updated !")
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deletelivingRoomFurnitures(id)
      if (response.status === 200) {
        console.log("livingRoomFurnitures Deleted!")

        toast.success("livingRoomFurnitures Deleted")
        fetchData()
      }
    } catch (error) {
      console.error(error)
  ...
  if (!livingRoomFurniture || livingRoomFurniture.length === 0) {
    return (
      <>
        <div className='w-screen h-[90vh] flex flex-col justify-center items-center'>
...

          {
            livingRoomFurniture.map((livingRoomFurnitures, index) => (
              <tr key={index}>
               ...
               hover:bg-blue-500 hover:text-white hover:shadow-blue-500'onClick={() => { editHelper(livingRoomFurnitures) }}>
                    <Pencil />
                  </button>
                  <button className='h-15 w-15 border-red-500 border-2 p-1 rounded-md text-red-500 shadow-md
               hover:bg-red-500 hover:text-white hover:shadow-red-500'onClick={() => handleDelete(livingRoomFurnitures._id)} >
                    <Trash />
                  </button>
                </td>
              </tr>
            ...
            <div className='h-[75%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  <h1 className='w-1/2 text-left text-xl my-6 font-bold text-green-500'>Add livingRoomFurnitures</h1>
        ...
        <>
          <div className="absolute top-0 left-0 z-50 h-screen w-screen flex justify-center items-center bg-black/40 ">
            <div className='h-[90%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                <h1 className='w-1/2 text-left text-xl my-6 font-bold text-blue-500'>Edit livingRoomFurnitures</h1>
                ...
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-8' onSubmit={handleEdit}>
                  ...
                  <input ref={delivery_timeRef} type="number" name="" id="Delivery_time" placeholder='Delivery_time' defaultValue={currentlivingRoomFurnitures.delivery_time} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-green-400 rounded-sm' required />                   <input ref={offerRef} type="number" name="" id="offer" placeholder='Offer'  defaultValue={currentlivingRoomFurnitures.offer} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-green-400 rounded-sm' required /><button type="submit" className="w-full h-[3rem]  shadow-lg shadow-gray-400 hover:shadow-blue-400 bg-blue-500 text-white rounded-sm outline-none">Save</button>
                </form>
              </div>
            </div>
          </div>
...
export default AdminLivingRoomFurniture